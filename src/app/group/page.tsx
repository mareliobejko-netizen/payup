...
import { changeRoleAction, createGroup, joinGroup, leaveGroupAction, regenerateInviteCodeAction, removeMemberAction, renameGroupAction, switchGroup } from "./actions";
...
export default async function GroupPage({ searchParams }: Props) {
  ...
  return <main className="min-h-screen bg-zinc-950 text-white"><div className="mx-auto min-h-screen max-w-md border-x border-white/5 px-5 py-5 pb-12">
    ...
    {active && <>
      ...
      <div className="mt-5 grid grid-cols-2 gap-3"><Link href="/group/activity" className="flex h-12 items-center justify-center gap-2 rounded-2xl bg-zinc-900 text-sm font-black"><History size={16}/>ATTIVITÀ</Link>{isAdmin&&<Link href="/group/settings" className="flex h-12 items-center justify-center gap-2 rounded-2xl bg-zinc-900 text-sm font-black"><Shield size={16}/>IMPOSTAZIONI</Link>}</div>
