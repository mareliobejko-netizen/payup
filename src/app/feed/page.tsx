...
export default async function FeedPage() {
  ...
  const posts = await db.select({ proofId: proofs.id, mediaUrl: proofs.mediaUrl, mediaType: proofs.mediaType, caption: proofs.caption, publishedAt: proofs.publishedAt, username: users.username, groupName: groups.name, penaltyTitle: penalties.title, likes: sql<number>`count(${proofLikes.id})` })
    .from(proofs)
    ...
    .groupBy(proofs.id, users.username, groups.name, penalties.title)
  ...
  return <main className="min-h-screen bg-zinc-950 text-white"><div className="mx-auto min-h-screen max-w-md border-x border-white/5 pb-28">
    ...
    <div className="space-y-5 px-5 py-6">{posts.length === 0 ? <div className="rounded-3xl border border-dashed border-white/10 p-10 text-center"><p className="text-5xl">🧱</p><h2 className="mt-4 text-xl font-black">The Wall è vuoto</h2><p className="mt-2 text-sm text-zinc-500">Quando qualcuno rende pubblica una prova approvata, apparirà qui.</p></div> : posts.map((post) => <article key={post.proofId} className="overflow-hidden rounded-3xl border border-white/10 bg-zinc-900">
      <div className="p-4"><div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-full bg-fuchsia-400 font-black text-black">{post.username[0]?.toUpperCase()}</div><div className="flex-1"><p className="font-black">@{post.username}</p><p className="text-xs text-zinc-500">{post.groupName}</p></div><span className="rounded-full bg-lime-400/10 px-3 py-1 text-[10px] font-black text-lime-300">✓ APPROVATA</span></div><p className="mt-4 rounded-2xl bg-zinc-800/70 p-3 text-sm font-semibold">💀 {post.penaltyTitle}</p></div>
