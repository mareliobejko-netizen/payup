...
import { ArrowLeft, Banknote, BadgeCheck, CalendarDays, Camera, Clock3, Skull } from "lucide-react";
    ...
    <main className="min-h-screen bg-zinc-950 text-white">
      <div className="mx-auto min-h-screen max-w-md border-x border-white/5 pb-10">
        ...
        <div className="space-y-6 px-5 py-6">
          ...
          <section className="grid grid-cols-2 gap-3"><InfoCard icon={<CalendarDays size={18} />} title="Assegnata" value={formatDate(penalty.createdAt)} /><InfoCard icon={<Clock3 size={18} />} title="Stato" value={statusLabel(penalty.status)} /></section>

          {penalty.status === "pending" && isAssignedUser && <section><div className="mb-4"><div className="flex items-center gap-2"><Camera size={20} className="text-lime-400" /><h3 className="text-lg font-black">L&apos;hai fatto?</h3></div><p className="mt-2 text-sm leading-6 text-zinc-500">Carica foto o video e lascia decidere al gruppo. 😈</p></div><ProofUploader penaltyId={penalty.id} /></section>}
...
function ProofCard({ proof, status }: { proof: typeof proofs.$inferSelect; status: "verifying" | "completed" }) { return <section className={`overflow-hidden rounded-3xl border bg-zinc-900 ${status === "completed" ? "border-lime-400/20" : "border-amber-400/20"}`}><div className="p-5"><span className={`rounded-full px-3 py-2 text-xs font-black uppercase ${status === "completed" ? "bg-lime-400/10 text-lime-300" : "bg-amber-400/10 text-amber-300"}`}>{status === "completed" ? "✅ Approvata" : "⏳ Da verificare"}</span><h3 className="mt-4 text-xl font-black">Prova inviata</h3></div>{proof.mediaType === "video" ? <video src={proof.mediaUrl} controls className="aspect-[4/3] w-full object-cover" /> : <img src={proof.mediaUrl} alt="Prova" className="aspect-[4/3] w-full object-cover" />}{proof.caption && <div className="p-5 text-sm text-zinc-300">“{proof.caption}”</div>}</section>; }
