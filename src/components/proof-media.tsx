...
export default function ProofMedia({ src, type, alt = "Prova", className = "aspect-[4/3] w-full object-cover" }: { src: string; type?: string | null; alt?: string; className?: string }) {
  const [open, setOpen] = useState(false);
  if (type === "video") return <video src={src} controls playsInline className={className} />;
  return <>
    ...
    {open && <div role="dialog" aria-modal="true" className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 p-3" onClick={() => setOpen(false)}>
      ...
      <img src={src} alt={alt} className="max-h-[94vh] max-w-[96vw] object-contain" onClick={(e)=>e.stopPropagation()}/>
    </div>}
  </>;
}
